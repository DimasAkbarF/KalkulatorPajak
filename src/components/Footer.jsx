...
export default function Footer({ onNavigate }) {
  ...
  return (
    <footer className="border-t border-slate-200 bg-white/70 px-4 py-10 transition-colors duration-500 ease-in-out dark:border-white/10 dark:bg-black/40 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-[1.5fr_1fr]">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600 transition-colors duration-500 dark:text-slate-400">
            Aplikasi sederhana untuk membantu menghitung estimasi PPh Final UMKM.
          </p>
          <p className="mt-5 max-w-2xl text-xs leading-5 text-slate-500 transition-colors duration-500 dark:text-slate-500">
          ...
          <div className="mt-3 flex flex-wrap gap-3 md:justify-end">
            {navItems.map((item) => (
              <a
                ...
                className="text-sm font-medium text-slate-500 transition-colors duration-300 hover:text-blue-800 dark:text-slate-400 dark:hover:text-white"
              >
                {item.label}
              </a>
            ))}
          </div>
          <p className="mt-6 text-xs text-slate-500 transition-colors duration-500 dark:text-slate-500">
