  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-center text-center">
        <div className="animate-fade-up inline-flex items-center rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-semibold text-blue-800 shadow-sm dark:border-blue-400/20 dark:bg-blue-400/10 dark:text-blue-200 dark:shadow-none">
          PPh Final UMKM • Estimasi Perhitungan • Siap Digunakan
        </div>

        <h1 className="animate-fade-up animation-delay-100 mx-auto mt-7 max-w-4xl text-4xl font-bold tracking-tight text-slate-950 sm:text-5xl lg:text-6xl dark:text-white">
          <span className="block">TIRTAX</span>
          <span className="mt-2 block text-3xl sm:text-4xl lg:text-5xl">KALKULATOR PAJAK UMKM</span>
        </h1>
        <p className="animate-fade-up animation-delay-200 mx-auto mt-5 max-w-3xl text-base leading-8 text-slate-600 sm:text-lg dark:text-slate-400">
          Dapatkan estimasi PPh Final UMKM 0,5% secara cepat, praktis, dan akurat berdasarkan omzet usaha.
        </p>

        <div className="animate-fade-up animation-delay-300 mx-auto mt-10 w-full max-w-md sm:max-w-3xl lg:max-w-4xl">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {stats.map((item, index) => (
              <div
                key={item.label}
                className={`rounded-3xl border border-slate-200 bg-white/85 p-4 text-left shadow-sm backdrop-blur transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-md dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none sm:p-5 ${
          ...
          <div className="animate-fade-up animation-delay-400 mt-8 flex justify-center">
            <button
              type="button"
              onClick={onStart}
              className="inline-flex min-h-12 w-full max-w-[220px] items-center justify-center rounded-xl bg-blue-800 px-6 py-3 text-sm font-semibold text-white shadow-sm transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-blue-100 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200 dark:focus:ring-white/20"
            >
              Mulai Hitung Pajak
            </button>
          </div>
